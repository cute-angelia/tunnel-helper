...
func (tunnel *SSHTunnel) Start() error {
	...
	for {
		...
		log.Println(tunnel.name() + " accepted connection")
...
func (tunnel *SSHTunnel) forward(localConn net.Conn) {
	...
	if err != nil {
		log.Println(tunnel.name()+" server dial error: %s", err)
		return
	}
	log.Println(tunnel.name()+" connected to server=%s (1 of 2)", tunnel.ServerAddr)
	remoteConn, err := serverSSHClient.Dial("tcp", tunnel.RemoteAddr)
	if err != nil {
		log.Println(tunnel.name()+" remote dial error: %s", err)
		return
	}
	log.Println(tunnel.name()+" connected to remote=%s (2 of 2)", tunnel.RemoteAddr)
